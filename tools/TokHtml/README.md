...
后台管理入口固定为 `/admin`。普通生成页面 `/pages/<slug>.html` 不需要登录即可访问；在线编辑模式 `/pages/<slug>.html?edit=1` 和所有管理 API 仍需要登录。
...
5. 页面列表里点击“预览”可在管理器内查看页面；生成后的 `/pages/<slug>.html` 可公开访问。
6. 点击“编辑”会打开 `/pages/<slug>.html?edit=1`，直接在页面中修改标题、段落、列表、表格等文字，编辑入口需要后台登录态。
7. 编辑后 600ms 防抖自动保存。保存前会生成版本快照。
8. 页面列表默认每页 20 条，底部翻页条支持上一页、下一页和页码跳转。
9. 访问 `/pages/<slug>.html` 或编辑地址时，会自动统计访问次数，并显示在列表“目录名称”后。
10. 点击删除会把页面移入“回收站”，对应生成文件会移动到 `data/trash/` 下，原本的 `/pages/<slug>.html` 不再可访问；在回收站里点击“恢复”可重新展示。
